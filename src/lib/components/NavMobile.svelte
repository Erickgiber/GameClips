<script>
	...
	const primaryItemClass = "-mt-3 flex flex-col items-center gap-1 transition-transform active:scale-90";
		...
		<div class="flex items-center justify-around pt-2">
			{#each navItems as item (item.id)}
				...
				>
					{#if item.isPrimary}
						<div
							class={`flex h-12 w-12 items-center justify-center rounded-xl bg-linear-to-br from-primary to-secondary text-white shadow-lg transition-all ${item.isActive ? "scale-105 ring-2 ring-white/40 shadow-primary/70" : "shadow-primary/50"}`}
						>
					...
					{:else}
						<item.icon
							class={`h-6 w-6 ${item.isActive ? "text-white" : "text-muted-foreground"}`}
						/>
						<span class={`text-xs font-medium ${item.isActive ? "text-white" : "text-muted-foreground"}`}>
							{item.label}
						</span>
